...
import { defaultPrefs, type ContentPrefs } from "./store-types";
...
function load(): Prefs {
  ...
  try {
    ...
    if (raw) return { ...base, ...JSON.parse(raw), content: { ...defaultPrefs, ...(JSON.parse(raw).content ?? {}) } };
...
export function StoreProvider({ children }: { children: ReactNode }) {
  const [prefs, setPrefs] = useState<Prefs>(() =>
    typeof window === "undefined"
      ? { fontSize: 16, highContrast: false, reducedMotion: false, hideArtwork: false, compactNarration: false, content: defaultPrefs, ambient: "off" as Ambient, muted: false, tutorialDone: false }
      : load()
  );
