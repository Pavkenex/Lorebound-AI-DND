...
from app.modules.play.state import PlayState, reopen_prologue_opening
...
def apply_creation(db: Session, campaign_id: str, state: PlayState) -> dict[str, Any]:
    ...
    pc["created"] = True
    # The chronicle may already be open on the prologue's road: the opening is
    # the player's own sheet, so it is handed back to the narrator the moment
    # the sheet lands (the seed writes no prose — P14).
    reopen_prologue_opening(state)
    db.commit()
    return {
        "applied": True,
        "character": pc,
